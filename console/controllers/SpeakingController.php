...
class SpeakingController extends BaseController
{
    ...
    {
        $local_path = dirname(__FILE__, 2);
        $file = $local_path . '/runtime/tmp/口语进阶气泡内容_2026年3月26日.json';
        $question_content = file_get_contents($file);
        $content = json_decode($question_content);
    ...
    {
        $local_path = dirname(__FILE__, 2);
        $file = $local_path . '/runtime/tmp/part2_splited_question_2026年3月26日.json';
        $question_content = file_get_contents($file);
        $content = json_decode($question_content);
        foreach ($content as $key => $value) {
            $id = $value->id;
    ...
    {
        $local_path = dirname(__FILE__, 2);
        $filename = $local_path . '/runtime/tmp/雅思口语新题_2026年3月26日.csv';
